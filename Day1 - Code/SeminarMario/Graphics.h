...
public:
	LivesGraphics(std::string const& path, int lCount);
public:
	virtual cv::Mat getCollisionMask();
	virtual void draw(cv::Mat& canvas, cv::Point const& topLeft);
...
public:
	ScoresGraphics(float fScale, int initialScore = 0, int fontFace = 0);
...
{
protected:
	Frame _applePic; // visualization of one life.

public:
	AppleGraphics(std::string const& path);
public:
	virtual cv::Mat getCollisionMask();
	virtual void draw(cv::Mat& canvas, cv::Point const& topLeft);
	// @2:reset: set number of lives:
